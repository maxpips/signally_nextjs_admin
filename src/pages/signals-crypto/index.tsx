...
export default function SignalsIndexPage() {
  const signalsOpen = useFirestoreStoreAdmin((state) => state.signalsCryptoOpen) || [];
  const signalsClosed = useFirestoreStoreAdmin((state) => state.signalsCryptoClosed) || [];
        ...
        <Page title='Maxpip'>
          <Container size='xl' className=''>
            <Box className='flex items-center justify-between mt-2 mb-5 text-center'>
              ...
              <div>
                <Link href='/signals-crypto/create?isAuto=false' className='mr-4'>
                  <Button
                    type='submit'
                    variant='white'
                ...
                <Link href='/signals-crypto/create?isAuto=true'>
...
function TableActions({ id }: { id: string }) {
  ...
  const handleDelete = async (modalId: string) => {
    ...
    try {
      if (id) await apiDeleteSignal({ id: id!, dbPath: 'signalsCrypto' });
  ...
  return (
    <Box className='flex'>
      <Edit size={20} className='mr-2 cursor-pointer text-app-yellow' onClick={() => router.push(`/signals-crypto/${id}`)} />{' '}
